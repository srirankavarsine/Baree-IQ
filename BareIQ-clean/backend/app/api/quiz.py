...
def submit_quiz(quiz_data: QuizSubmit, db: Session = Depends(get_db)):
    ...
    if quiz_data.is_guest or not quiz_data.email:
        user = User(guest_id=str(uuid.uuid4()))
        db.add(user)
    else:
    ...
    skin_profile = (
        db.query(SkinProfile).filter(SkinProfile.user_id == user.id).first()
    )
    if not skin_profile:
    ...
    # Replace old recommendation rows so results match the latest quiz.
