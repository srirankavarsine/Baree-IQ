...
class RecommendationEngine:
    ...
    def calculate_score(
        ...
        # Guard against missing profile fields so scoring never 500s.
        skin_type = (skin_profile.skin_type or "").strip().lower()
        skin_tone = (skin_profile.skin_tone or "").strip().lower()

        # Skin type match (30 points)
        product_skin_types = RecommendationEngine._normalize_terms(product.skin_types or "")
        if skin_type and (skin_type in product_skin_types or "all" in product_skin_types):
        ...
        if product.skin_tones:
            ...
            if skin_tone and (skin_tone in product_tones or "all" in product_tones):
                score += RecommendationEngine.WEIGHTS["skin_tone"]
            elif "all" in product_tones:
                score += RecommendationEngine.WEIGHTS["skin_tone"]
        else:
